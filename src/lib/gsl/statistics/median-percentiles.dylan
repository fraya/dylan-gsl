...
define function median
 ...
 => (median :: <double-float>)
  if (sorted?)
    gsl-stats-median-from-sorted-data(data.%gsl-vector-data, data.gsl-vector-stride, data.size)
  else
    gsl-stats-median(data.%gsl-vector-data, data.gsl-vector-stride, data.size)
  end if;
end;